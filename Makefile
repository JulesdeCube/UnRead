...
SRC=main.c load_Image/load.c load_Image/transform_BW.c
DEPS=load_Image/load.h load_Image/transform_BW.h
BUILD=UnRead-0.1.0

CC=gcc
GTK_CFLAGS=$(shell pkg-config --cflags gtk+-3.0)
GTK_LIB=$(shell pkg-config --libs gtk+-3.0)
CFLAGS=-I$(SRC_DIR) $(GTK_CFLAGS) -Wall -Wextra -Werror -std=c99 -pedantic -g $(GTK_LIB)
